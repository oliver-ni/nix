...
or uploaded enough and are not among the last seeders, drop downloads that
never got going, then ask Prowlarr for the newest releases and add the ones
worth seeding. Each grab is independent; a failure is logged and the run moves
on. Info hashes already handled are kept in STATE_DIRECTORY so a deleted or
skipped torrent is not considered again.
"""
...
def cleanup(torrents, now):
    ...
    still under 10% with no transfer for STALL_TIME (AvistaZ lets those go
    ...
    for t in torrents:
        ...
        if t["progress"] < 0.1 and idle >= STALL_TIME.total_seconds():
            log(f"stalled at {t['progress'] * 100:.1f}% for {idle / 3600:.1f}h: {t['name']}")
...
def main():
    ...
    ours = cleanup([t for t in torrents if t["category"] == CATEGORY], now)
