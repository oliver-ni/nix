...
- `ochazuke.org` serves `oliver-ni/jellyfin-client` (flake input over SSH, built with `buildNpmPackage`; refresh `npmDepsHash` with `prefetch-npm-deps package-lock.json` when the lockfile changes). ochazuke has no GitHub SSH key, so run `nix copy` of the input's store path to it before `nixos-rebuild` after bumping the input. `/seerr/*` is proxied to Seerr with the prefix stripped; `requests.` keeps Seerr's own UI for admin; `jellyfin.` keeps the API for clients and the arrs.
