...
- Sonarr and Radarr ship with file renaming **off**; it is enabled with TRaSH's anime/movie formats (set through their APIs, not Nix). Raw batch filenames like `Show -77.mkv` make Jellyfin mis-season episodes. `RenameFiles` takes one `seriesId`/`movieId` per command.
