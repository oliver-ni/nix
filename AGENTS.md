...
- Jellyfin is trialing SleekFin through its plugin manager with File Transformation; this plugin state is not declared in Nix. The unused `modules/nixos/jellyfin-theme.nix` preserves Abyss for rollback: disable SleekFin, restore that import, and restore `@import url("ui/abyss.css");` in branding (POST `/System/Configuration/Branding`). Do not run both themes together.
